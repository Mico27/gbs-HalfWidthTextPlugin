const id = "EVENT_HWT_DISPLAY_DIALOGUE";
const name = "Half-Width Text: Display Dialogue";
const groups = ["EVENT_GROUP_DIALOGUE"];
const autoLabel = (fetchArg) => {
  return `Half-Width Text: Display Dialogue`;
};
const fields = [
  {
    key: "__section",
...
const compile = (input, helpers) => {
  ...
  _addComment("Half-Width Text Dialogue");
  if (renderOnTop) {
    _stackPushConst(0);
  ...
  texts.forEach((text, textIndex) => {
    ...
    const decoratedText = `${textPosSequence}${_injectScrollCode(text, textHeight)}`;
    _loadStructuredText(decoratedText);
    _invoke("hwt_display_dialogue", 0, ".ARG0");
    if (isModal) {
